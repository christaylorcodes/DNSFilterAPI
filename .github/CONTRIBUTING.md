...
This module uses a Sampler / ModuleBuilder build. To get a local environment ready:

-   **First-time setup** — resolve build dependencies: `./build.ps1 -ResolveDependency`
-   **Build the module:** `./build.ps1 -Tasks build`
-   **Run the test suite:** `./build.ps1 -Tasks test`
-   **Run the linter (PSScriptAnalyzer):** `./build.ps1 -Tasks lint`

### Pull Requests

Before you push and open a pull request, run the local validation script to mirror what CI checks:

-   **Pre-push validation:** `./Tests/test-local.ps1` (build + analyze + test)

Make sure all of the above pass and that PSScriptAnalyzer reports zero errors before requesting review.

## AI Contributors

This project welcomes contributions from AI coding agents. AI agents follow the same quality standards as human contributors. See [CLAUDE.md](../CLAUDE.md) for complete instructions, conventions, and the issue-driven workflow.
